...
const tasksSlice = createSlice({
  ...
  reducers: {
    ...
    addDirectTask: (state, {payload}: PayloadAction<ITask>) => {
      return {
        ...state,
        directTasks: [
          ...state.directTasks,
    ...
    removeDirectTask: (state, {payload}: PayloadAction<string>) => {
      return {
        ...state,
        directTasks: state.directTasks.filter(item => item.uuid !== payload)
      }
    },
    editDirectTask: (state, {payload}: PayloadAction<ITask>) => {
      return {
        ...state,
        directTasks: state.directTasks.map(item => item.uuid !== payload.uuid ? item : payload)
      }
    },
  }
})
