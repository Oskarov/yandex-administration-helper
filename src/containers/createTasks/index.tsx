...
import {
    createTasksInSprint,
    getAllBoardsAction, getAllQueuesAction,
    getAllSprintsByBoardId, getAllTasksByQueueKey,
...
export const CreateTasks: FC = ({}) => {
    ...
    const [targetQueue, setTargetQueue] = useState(localStorage.getItem('lastTargetQueue') ? localStorage.getItem('lastTargetQueue') : '');
    ...
    const handleChange = () => {
        if (chosenSprint && !!targetQueue) {
            dispatch(createTasksInSprint(chosenSprint, trackerIds.map(i=>{
    ...
    return <div>
        <div className={styles.dropContainer}>
            <TextField id="outlined-basic" label="Ключ Очереди задач (TMS)" variant="outlined" value={targetQueue}
        ...
        {!!chosenSprint && <div className={styles.performers}>
            {performers.filter(i => !!i.trackerId).map(performer => <div key={performer.uuid}
                                                                         className={styles.performerItem}
                                                                         onClick={() => performer.trackerId ? handlePerformer(performer.trackerId) : null}>
                <Checkbox checked={performer?.trackerId ? trackerIds.includes(performer.trackerId) : false}/>
                <div>{`${performer.lastName} ${performer.firstName}`}</div>
            </div>)}
        </div>}

        {
            trackerIds.length > 0 && targetQueue && <div>
                <div className={styles.name}>
                    <div>{chosenSprintText} -</div>
                    <TextField id="outlined-basic" label="Часть названия" variant="outlined" value={name}
                ...
                <div className={styles2.sprintButtons}>
                    <Button onClick={handleChange} color={'primary'} variant={'contained'}
                            disabled={trackerIds.length < 1 && name.length < 1}>Создать</Button>
                </div>
            </div>
        }
