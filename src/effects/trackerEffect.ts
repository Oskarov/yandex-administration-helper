...
export const createTasksInSprint = (targetSprintId: number, performersData: {taskName: string, id: number}[], queueKey:string ) => {
    return async function (dispatch: Dispatch<any>) {
        ...
        performersData.forEach((task, idx) => {
            setTimeout(async () => {
                ...
                const response = await SprintService.createTaskForSprint(task.id, task.taskName, targetSprintId, queueKey);
                if (!response) {
                    console.log('bad');
                }
                if (idx + 1 >= tasksCount) {
